...
import {productSchema, reviewSchema} from '@/utils/schemas';
...
export const createProductAction = async (prevState: unknown, formData: FormData): Promise<{
  message: string
}> => {
  const user = await getAuthUser();
  try {
